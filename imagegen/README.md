...
4. If the first try is rejected, one retry is made with another seed. If that also fails (gate or vision QA), the job is reported `estado: error`, `error: edit_gate_failed`, `retryAfterSeconds: 3600`, so a later run retries with new seeds (the backend caps attempts at 5, then the panel shows "Regenerar"). A quota error reports `quota_wait` (no attempt used); a provider failure reports `edit_provider_error` (retry in 15 min). The worker never claims jobs while the edit provider is unavailable. Gate numbers for every attempt go in `qa.edit`, and `provider` is `cfedit:<model>`. The composite flow below is **disabled** unless `IMAGEGEN_ALLOW_COMPOSITE=1` (the owner rejected that style; `--dry-run` still writes composites for inspection).
